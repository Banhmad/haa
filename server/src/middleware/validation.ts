...
export const validateRegister = (req: Request, _res: Response, next: NextFunction): void => {
  ...
  if (!isValidUsername(sanitizeInput(username))) return next(createError('Username must be 3-30 alphanumeric characters', 400));
...
export const validateProfileUpdate = (req: Request, _res: Response, next: NextFunction): void => {
  ...
  if (username && !isValidUsername(sanitizeInput(username))) return next(createError('Username must be 3-30 alphanumeric characters', 400));
  if (bio !== undefined) {
